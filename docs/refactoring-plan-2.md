...
### Фаза L — реєстр `figureForSrc` для навчальних фігур (🟡) — ⏭️ ПРОПУЩЕНО (після аналізу коду)
...
> **Рішення: ПРОПУЩЕНО (після читання коду — рішення користувача).** Кожен `figureForSrc`
> складається з ~3 спільних рядків (розбір stem: `split('/').pop()` + зняти розширення +
> `caption`, та `default → <FigureCard>`) і ~80 рядків **суто per-algo**: імпорти власних
> віджетів + regex-діспетчі (`step_NN`, `step_main_NN`) з РІЗНИМИ віджетами, пропсами й
> формулами індексів (linear `2K+2`, bubble `N+1`, …) + `switch` за іменами, де кожен case →
> свій віджет зі своїми еталонними даними. План переоцінив зиск: «реєстр» не дедуплікує switch
> — лише ПЕРЕНІС би його у конфіг-мапу того самого розміру (regex-кейси зробив би незручними)
> + додав шар непрямості до навмисно самодостатніх навчальних мапінгів. Реально спільне —
> тривіальний розбір stem (як `fmt`/runtime-білдер у K/M, які теж пропущено). Негативний ROI →
> лишаємо `figureForSrc` плаский і per-algo.
...
| L | реєстр figureForSrc | 2 | 🟡 | ⏭️ ПРОПУЩЕНО (switch незводно per-algo) |
...
Виконано H, I, J, K, N, M (+ позаплановий фікс краша `indexedSequentialSearch` на порожньому
масиві). Чисто прибрано ≈ **1781 рр.** дубльованого/боілерплейтного коду при незмінній
поведінці й тих самих тестах (1127 зелених на кожному кроці):

| Фаза | Зиск |
|---|---|
| H — фабрики сторів | −322 |
| I — спільний ArrayEditor | −495 |
| J — playback-kit | −458 |
| K — спільний prng (fmt/assertTraceInvariants пропущено) | −36 |
| N — хелпер createAlgorithm | −422 |
| M — базові Frame-інтерфейси (runtime-білдер пропущено) | −48 |

**Свідомо НЕ зроблено** (після аналізу коду — переоцінений у плані зиск / негативний ROI /
проти самодостатності навчального коду): `fmt`, `assertTraceInvariants` (у K), runtime
`SearchTraceBuilder` (у M), **L — реєстр figureForSrc** (switch незводно per-algo). **O (i18n)**
— відкладено (Tier 3). Висновок: великі структурні дублікати (стори/редактори/плеєри/реєстр)
прибрано; решта «дублювання» — або тривіальна (1-рядкові хелпери, LOC-нейтральні), або
насправді алгоритмо-специфічний вміст, який дедуплікація лише замаскувала б.
