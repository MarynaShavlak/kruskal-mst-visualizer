...
### Фаза K — дрібні lib-утиліти (🟢; нульовий ризик, незалежна) — ✅ ВИКОНАНО (звужено до prng)
...
> **Підсумок виконання (звужено).** Зроблено лише **prng**: `lib/prng.ts` — єдине джерело
> `mulberry32`; 4 рядкові пресети (naive/kmp/boyer-moore/rabin-karp) імпортують звідти
> замість локальних копій, `randomGraph` ре-експортує для сумісності зі старими імпортами
> (randomArray/randomKnapsack/randomTsp/randomDirectedGraph не чіпано). Послідовність PRNG
> байт-у-байт та сама (детермінізм `loadRandom` збережено). **Чисто −36 рр.** `tsc -b` чистий
> · `npm test` 1127/1127 · прод-білд ОК.
>
> **Свідомо НЕ зроблено** (оцінено й відкинуто як низькоцінне — за принципом «не переінженерити»):
> - **спільний `fmt`** — формат «[1, 2, 3]» тривіальний і стабільний; 10 копій-однорядківців,
>   але винесення LOC-нейтральне (рядок `const` → рядок `import` у кожному файлі) за мізерний
>   зиск + помітний churn (10 файлів). Лишено локально.
> - **`assertTraceInvariants`** — форми кадрів надто різнорідні (`i` vs `index`; «останній» =
>   `done`/`final`/`step==="done"`; `phase` vs `sub.kind`), а інваріанти в тестах уплетені в
>   алгоритмо-специфічний контекст (монотонність лічильника, незмінність масиву). Generic-хелпер
>   вимагав би per-algo конфіга й знизив би специфічність наявних асертів — негативний ROI.
