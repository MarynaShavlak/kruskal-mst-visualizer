...
## Фаза F — редактор (🟠 вищий ризик; останнім, можна відкласти) — ◑ ЧАСТКОВО
...
- **Підкрок 2 — хук-контролер синку store↔React-Flow — ВІДКЛАДЕНО** (ризикованіше,
  у `EditorView` немає рендер-тестів).

**Коміт:** `Рефакторинг: спільна серіалізація графа в редакторі (graph-doc codec)`
...
| F | редактор: graph-doc codec ✅; контролер відкладено | 🟠 | ◑ ЧАСТКОВО |
