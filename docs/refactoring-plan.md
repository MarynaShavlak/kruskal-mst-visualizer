...
## Фаза F — редактор (🟠 вищий ризик; останнім, можна відкласти)
Виносимо лише невізуальне: `graph-doc` (серіалізація + шаринг через URL-хеш `?g=`, JSON import/export),
опційно хук-контролер синку store↔React-Flow. Рендерери вузлів/ребер, бокові панелі й пресети — по-алгоритмні.

**Коміт:** `Рефакторинг: спільна серіалізація/шаринг графа в редакторі`
