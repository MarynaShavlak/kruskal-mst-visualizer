...
import { ConnectivityPanel } from "@/features/editor/ConnectivityPanel"
import {
  decodeHash,
  encodeHash,
  fromJSON,
...
function promptWeight(current?: number): number | null {
  const raw = window.prompt(
    "Вага ребра (додатне ціле):",
    current != null ? String(current) : "1",
  )
  if (raw === null) return null
  const w = Number.parseInt(raw, 10)
  if (!Number.isInteger(w) || w <= 0) {
    window.alert("Вага має бути додатним цілим числом.")
    return null
  }
  return w
}
...
function EditorCanvas() {
  ...
  const onShare = useCallback(() => {
    ...
    const url = `${window.location.origin}${window.location.pathname}#editor?g=${hash}`
    window.location.hash = `editor?g=${hash}`
