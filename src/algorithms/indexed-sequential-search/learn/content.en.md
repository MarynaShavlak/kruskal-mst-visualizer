...
Here are the two functions from the notes — the ones we walk through line by line (the fully documented versions are in [`indexed_sequential_search/core.py`](indexed_sequential_search/core.py)):
...
The console summary and the **runnable example from the notes** — the very same output (printed by [`examples/01_intro.py`](examples/01_intro.py); the program output line is in Ukrainian, matching the source notes byte for byte):
...
The runnable example from the notes:
...
- **`indexed_sequential_search`** — the base version: phase 1 is a **binary** search over the index (as in the notes), `O(log n)`.
