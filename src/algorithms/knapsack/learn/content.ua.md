...
> **Про код.** Базові реалізації наведено **дослівно** (коментарі включно). Зазвичай усі три підходи називаються однаково — `knapSack`; вони мусять співіснувати, тож функціям дано різні імена: `knapsack_recursive`, `knapsack_brute_force`, `knapsack_greedy`, `knapsack_dp`. Тіла функцій не змінювалися.
...
Далі в розборі працюватимуть **два інстанси**:
...
## 2. Суть методу повного перебору (Brute-force)
...
## 3. Усі комбінації для нашої задачі
...
Та сама таблиця, але рисунком — смуга кожної підмножини показує її вагу на тлі місткості (пунктир), праворуч вердикт:
...
## 4. Базова реалізація — рекурсія «взяти / не брати»
...
## 7. Порядок виконання коду (трасування)
...
## 8. Явний перебір підмножин (`itertools`)

Рекурсія перебирає комбінації неявно — через розгалуження викликів. Той самий перебір можна записати «в лоб»: згенерувати всі підмножини індексів і перевірити кожну. Бонус: ця версія повертає не лише вартість, а й **сам набір**. Код:
...
Обидві версії дають ту саму відповідь:

```text
Рекурсивна версія: 220
...
Обидва підходи чесно проганяються на цьому інстансі (20 предметів, `W = 60`, числа зафіксовано наперед):
...
## 10. Динамічне програмування: шпаргалка з підзадач
...
## 12. Базова реалізація — таблиця `K[i][w]`
...
## 13. Чому при «не вміщується» беремо значення зверху
...
## 14. Покрокове заповнення таблиці (малий інстанс)

Щоб кожна клітинка вмістилася на екран, переходимо на **малий інстанс**: рюкзак на **4** одиниці, предмети П1 (вага 1, варт. 6), П2 (вага 2, варт. 10), П3 (вага 3, варт. 12). Таблиця — лише 4×5.
...
## 15. Найцікавіші клітинки — формула в кадрі
...
## 16. Зведена картина: еволюція таблиці
...
## 17. Відповідь і відновлення набору зворотним проходом
...
> У цьому фрагменті таблиця називається `dp` — це та сама `K` (обидві назви трапляються в літературі). І ще одна відмінність: цей фрагмент складає список **імен** (`names[i - 1]`), а пакетна `reconstruct_items` повертає **0-базові індекси** обраних предметів — імена до них додають уже приклади.
...
## 18. Класичний інстанс: таблиця для W = 50

Повернімося до початкової задачі (`W = 50`, ваги `[10, 20, 30]`). Таблиця тут — 4 рядки × **51 стовпець**, але оскільки всі ваги кратні 10, значення змінюються лише на кратних 10 місткостях. Тому показуємо зведену версію:
...
## 19. Покрокове виконання коду: панелі «код ↔ таблиця»

Розділи вище показували *результат* кожного кроку — як «дозріває» таблиця. Тут — **сам код у дії**: ліворуч фрагмент алгоритму з **підсвіченими активними рядками** (🟨 рядок, що виконується зараз; тьмяніше — охопні цикли `for`), праворуч — стан таблиці `K` саме на цьому кроці. **Колір активної клітинки таблиці кодує, яка гілка спрацювала:** 🟩 перемогла гілка «взяти», 🟦 «не брати» (значення зверху), ⬜ предмет не влазить, сіра — базовий випадок. Натискай ▶ або «крок», щоб пройти виконання вручну.

> У кодовій панелі гілки `max(...)` розписані окремими рядками — «взяти» (рядок 7) і «не брати» (рядок 8), — щоб кожну можна було підсвітити окремо.

Обидва рівні деталізації будуються з одного trace ([`buildKnapsackTrace`](src/lib/knapsackTrace.ts)), що ним живиться і вкладка «Алгоритм».

### Огляд: увесь прогін на малому інстансі

Зовнішній цикл `for i` додає предмети рядок за рядком, а права панель заповнює таблицю клітинка за клітинкою; наприкінці код переходить до відновлення набору (рядки 11–15):

![Код ↔ таблиця: повний прогін (малий інстанс)](docs/images/code_steps_small.png)

### Детально: крок = одна клітинка

Для найпоказовішого рядка (`i = 3` — у ньому трапляються **всі** гілки переходу: база, «не влазить», «не брати» і «взяти») лишаємо лише його кадри — повний скан клітинок `w = 0…4`:

![Код ↔ таблиця по клітинках (рядок i=3)](docs/images/code_walk_small_i3.png)

### Той самий код на класичному інстансі

Код не змінився ні на символ — змінилися лише дані (`W = 50`, таблиця тут — 4×51 клітинки):

![Код ↔ таблиця: повний прогін (класичний інстанс)](docs/images/code_steps_classic.png)

## 20. Порівняння трьох підходів

Усі три методи на **одному** класичному інстансі:
...
- **повний перебір** — коли предметів зовсім мало (до ~20) або потрібен незаперечний еталон для перевірки інших методів;
...
## 21. Обмеження 1: жадібний метод не розв'язує задачу 0/1
...
Порядок після сортування не змінюється: П1 (6.0) → П2 (5.0) → П3 (4.0). Далі:
...
## 22. Обмеження 2: дуже великий W (псевдополіноміальність)
