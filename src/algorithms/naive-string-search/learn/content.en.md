...
A summary across all 11 alignments (printed by [`examples/01_intro.py`](examples/01_intro.py)) — you can see how many comparisons each one "eats":
...
We build this for the canonical example; generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). Each grid row is one alignment (its verdict):
