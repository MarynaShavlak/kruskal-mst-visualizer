...
This repository is a learning resource: a clean implementation of the algorithm + detailed visualizations of every step. The entire walkthrough below is reproduced by the code in [`examples/`](examples), and the figures live in [`docs/images/`](docs/images).
...
Commands for installing, running the examples and tests, and a minimal library-usage example are in **[USAGE.en.md](USAGE.en.md)**.

## 3. Intuition: arranging cards in hand
...
## 4. The idea: prefix, key and shifts
...
## 6. Example — the array `[5, 2, 4, 6, 1, 3]`
...
### The base implementation (the code from the notes)

Here is the implementation from the notes — the one we dissect line by line (the full documented version is in [`insertion_sort/core.py`](insertion_sort/core.py)):
...
The learning version [`insertion_sort_steps`](insertion_sort/core.py) repeats this code **action for action**, but after every meaningful action it records a snapshot of the array state and the comparison and shift counters — all the pictures below are assembled from these snapshots.
...
The complete trace of all iterations (printed by [`examples/01_intro.py`](examples/01_intro.py)):
...
## 7. Adaptive without a flag: best and worst cases
...
## 8. Binary insertion: fewer comparisons, the same shifts

The “optimization” of insertion sort is **not** an early-exit flag (the inner `while` already stops early). Instead, we can speed up the **search for the slot**: the sorted prefix lets us find the insertion position with a **binary search** instead of a linear one. This is **binary insertion sort** ([`insertion_sort_binary`](insertion_sort/core.py)).

The win concerns only **comparisons** — they become $O(n\log n)$ instead of $O(n^2)$. But the **shifts stay** $O(n^2)$: the elements still have to be physically moved right to free the slot. On reverse input the gap in comparisons grows with the array size (printed by [`examples/02_optimized.py`](examples/02_optimized.py)):
...
## 10. Executing the code step by step: code ↔ array panels
...
We build this for the array from the notes `[5, 3, 8, 4, 2]` (its line-by-line trace matches the one in the notes); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). Each grid row is a step of the algorithm (taking the key, the shift / “spot found” branch, the insertion):
...
## 11. The full step-by-step trace of `[5, 2, 4, 6, 1, 3]`
...
## 13. Limitations: why $O(n^2)$ is impractical for large `n`
...
## 14. Where it fits: insertion in Timsort/introsort
...
## 15. Comparison with bubble and selection sort
