...
Here is the base implementation — the one we dissect line by line (the full documented version is in [`insertion_sort/core.py`](insertion_sort/core.py)):
...
We build this for the example array `[5, 3, 8, 4, 2]` (its line-by-line trace matches the walkthrough above); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). Each grid row is a step of the algorithm (taking the key, the shift / “spot found” branch, the insertion):
