...
This repository is a learning resource: a clean implementation of the algorithm plus detailed visualizations of every step. The whole walkthrough below is reproduced by the code in [`examples/`](examples), and the figures live in [`docs/images/en/`](docs/images/en).
...
The install commands, how to run the examples and the tests, plus a minimal library-usage snippet are in **[USAGE.en.md](USAGE.en.md)**.

## 3. Intuition: divide and conquer
...
## 4. The idea: split in half and merge
...
## 5. Why the tree is always balanced
...
## 6. Example — the array `[8, 4, 6, 2, 7, 1, 5, 3]`
...
Here is the implementation from the notes — the one we walk through line by line (the fully documented version is in [`merge_sort/core.py`](merge_sort/core.py)):
...
The instrumented version [`merge_sort_steps`](merge_sort/core.py) replays this code **step for step**, but builds a **journal/tree** of split and merge events together with counters of comparisons, appends and depth — and all the pictures below are assembled from those events.
...
The same thing as a step-by-step trace (printed by [`examples/01_intro.py`](examples/01_intro.py)) — the recursion level, the function, the subarray, `mid` and the two halves. The `merge_sort` rows go top-down (the split), the `merge` rows go bottom-up (the merge), in actual execution order:
...
🎬 *MP4 version:* [`recursion_intro.mp4`](docs/images/en/recursion_intro.mp4)
...
🎬 *MP4 version:* [`merge_step.mp4`](docs/images/en/merge_step.mp4) · also the animation of merging the root halves — [`merge_root.mp4`](docs/images/en/merge_root.mp4)
...
## 7. The $O(n\log n)$ guarantee with no degeneration

The most important property of merge sort is the **guaranteed** $O(n\log n)$ in **all** cases. Let's look more closely at the merge operation itself and compare the growth with quicksort (generated by [`examples/02_merge_step.py`](examples/02_merge_step.py)):
...
To see it, take "tagged" duplicates — each copy of a value carries a subscript showing its original spot (`3₁`, `3₂`, `3₃`). Generated by [`examples/03_stability.py`](examples/03_stability.py):
...
## 9. Top-down vs. bottom-up → the bridge to TimSort

The version above is **top-down recursive**: it splits the array from the top down and merges on the way back up the recursion. The same algorithm can be written **without recursion** — the **bottom-up** way: treat each element as a sorted "run" of length 1 and merge adjacent runs into runs of length 2, then 4, 8, … until one run covers the whole array (the full implementation is [`merge_sort_bottom_up`](merge_sort/core.py)):
...
We build this for the array from the notes `[5, 3, 8, 4, 2]` (its line-by-line trace matches the walkthrough in the notes); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). The tree trace for this array is exactly as in the notes:
...
🎬 *MP4 version:* [`code_walk_conspect.mp4`](docs/images/en/code_walk_conspect.mp4) · the recursion tree of the notes' array — [`docs/images/en/tree_conspect.png`](docs/images/en/tree_conspect.png)

## 11. Full step-by-step trace of `[8, 4, 6, 2, 7, 1, 5, 3]`
...
## 13. Limitations: the memory cost and non-adaptivity
...
## 15. Series finale: TimSort and the whole sorting picture
