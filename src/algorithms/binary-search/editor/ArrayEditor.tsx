 ...
 * двійкового пошуку — масив відсортований (стан показує панель збоку; є кнопка
...
export function ArrayEditor({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("flex flex-col gap-3", className)}>
      ...
      {/* Масив. */}
      {values.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("editor.binNoValues")}</p>
      ) : (
          ...
          {values.map((v, i) => (
            <div key={i} className="flex flex-col items-center gap-1">
              ...
              <div className="group relative">
                <NumberField
                  value={v}
                  aria={t("editor.binAriaValue", { i })}
                  onCommit={(n) => updateValue(i, n)}
                />
                <button
                  type="button"
                  aria-label={t("editor.binDeleteAt", { i })}
