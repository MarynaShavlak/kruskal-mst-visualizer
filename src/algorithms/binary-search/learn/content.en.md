...
The instrumented version [`binary_search_steps`](binary_search/core.py) repeats this code **step for step**, but after every comparison it records a snapshot of the window `[low..high]` and the probe `mid` — all the pictures below are assembled from those snapshots.
...
The same walkthrough as a table `low/high/mid/arr[mid]/x/found?` (printed by [`examples/01_intro.py`](examples/01_intro.py)) — all indices are **absolute** (into the original array), so the last `mid` for a found value is the answer:
...
Together they give the half-open range of all occurrences `[lower_bound, upper_bound)`, and hence their count. Printed by [`examples/03_variants.py`](examples/03_variants.py):
...
We build this for the main instance (searching for 15); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). Each row of the grid is one decision (you can see which branch fired and how the window shrank):
...
| **Binary search** *(this repository)* | search by halving (sorted data) | $O(\log n)$ |
