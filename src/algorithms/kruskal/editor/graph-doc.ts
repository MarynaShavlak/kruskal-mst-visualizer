// Серіалізація документа редактора (граф + позиції):
//  - JSON для імпорту/експорту;
//  - компактний base64url для шарингу через URL-хеш.
// Імпорт валідує структуру і перебудовує граф через lib/graph (інваріанти).
...
function docToWire(doc: GraphDoc): WireDoc {
  return {
    version: 1,
    vertices: [...doc.graph.vertices],
    edges: doc.graph.edges.map((e) => [e.u, e.v, e.weight] as const),
...
function wireToDoc(wire: WireDoc): GraphDoc {
  let g: Graph = emptyGraph()
  for (const v of wire.vertices) g = addVertex(g, v)
  for (const [u, v, w] of wire.edges) g = addEdge(g, u, v, w)
...
export function toJSON(doc: GraphDoc): string {
  return JSON.stringify(docToWire(doc), null, 2)
}

export function fromJSON(json: string): GraphDoc {
...
export function encodeHash(doc: GraphDoc): string {
  return toBase64Url(JSON.stringify(docToWire(doc)))
}

export function decodeHash(s: string): GraphDoc | null {
  try {
