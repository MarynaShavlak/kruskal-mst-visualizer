...
This repository is educational material: a clean implementation of the algorithm (verbatim from the source notes) plus detailed visualizations of every step of both phases. The whole walkthrough below is reproduced by the code in [`examples/`](examples), and the figures live in [`docs/images/en/`](docs/images/en).
...
Installation, running the examples and tests, and a minimal library-usage example are in **[USAGE.en.md](USAGE.en.md)**.

## 3. Intuition: why re-comparisons are wasteful
...
## 5. Phase 1 — the prefix function (the `lps` table)
...
## 11. Place in the series of string algorithms
