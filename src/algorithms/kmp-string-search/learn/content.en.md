...
Here is the base implementation — the very one we walk through line by line:
...
Consider the pattern `ABAB`. Here is the table:
...
Let us cement the idea on the pattern `AABAA` — it shows the most interesting step: the **fall-back** `length = lps[length-1]`. The full trace:
...
The substring «алг» starts at position 4: `Ц`(0), `е`(1), `й`(2), `␣`(3), `а`(4), `л`(5), `г`(6). The full trace:
...
Let us compare the number of character comparisons on the same inputs:
