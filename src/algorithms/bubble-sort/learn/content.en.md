...
The instrumented version `bubble_sort_steps` repeats this code **action for action**, but after every comparison it records a snapshot of the array and the comparison/swap counters — all the pictures below are built from those snapshots.
...
The full journal of all passes:
...
We build this for the example array `[5, 3, 8, 4, 2]` (its line-by-line trace matches the walkthrough above). Each grid row is one comparison (both branches of the `if` are visible: swap / keep):
