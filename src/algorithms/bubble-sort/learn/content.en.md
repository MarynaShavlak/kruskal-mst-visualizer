...
Here is the base implementation — the one we dissect line by line (the fully documented version is in [`bubble_sort/core.py`](bubble_sort/core.py)):
...
We build this for the example array `[5, 3, 8, 4, 2]` (its line-by-line trace matches the walkthrough above); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). Each grid row is one comparison (both branches of the `if` are visible: swap / keep):
