...
Here is the base implementation — the one we walk through line by line (the fully documented version is in [`shell_sort/core.py`](shell_sort/core.py)):
...
We build this for the example array `[5, 3, 8, 4, 2]` (phases `gap = 2`, then `gap = 1`); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py):
...
The same array also gives the **instrumented trace** — the verbatim terminal output of the `print` version (printed by the same [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py)). It is kept verbatim (Ukrainian print messages):
