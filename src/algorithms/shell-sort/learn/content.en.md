...
The instrumented version `shell_sort_steps` repeats this code **action for action**, but after each significant event it records a snapshot of the array and the comparison/shift counters — every picture below is built from those snapshots.
...
The full trace by phase — you can see that the early phases do only a few far-apart shifts, while the final `gap = 1` phase merely tidies up:
...
Why `n//2` is poor: its consecutive gaps are not coprime (`8, 4, 2, 1` are all powers of two), so elements from even and odd positions stay "unmixed" for a long time. Knuth and Ciura avoid this. The parameterized implementation `shell_sort_with_gaps` with the generators `gaps_shell`/`gaps_knuth`/`gaps_ciura` lets us compare them on the same data:
...
We build this for the example array `[5, 3, 8, 4, 2]` (phases `gap = 2`, then `gap = 1`):
...
The same array also gives the **instrumented trace** — the verbatim terminal output of the `print` version. It is kept verbatim (Ukrainian print messages):
