...
This repository is teaching material: a clean implementation of the algorithm (verbatim from the source notes) + detailed visualizations of every step of both phases. The entire walkthrough below is reproduced by the code in [`examples/`](examples), and the figures live in [`docs/images/en/`](docs/images/en).
...
## 3. Intuition: why scan from the end
...
## 4. The two phases of Boyer-Moore
...
## 5. Phase 1 — the bad-character table (shift table)
...
The same step by step (printed by [`examples/01_shift_table.py`](examples/01_shift_table.py)) — you can see the two **overwrites** of `e` and the `setdefault` for `r`:
...
## 7. Boyer-Moore vs naive and KMP
...
## 8. Code execution step by step: «code ↔ data» panels

The examples above showed the *result* of each step. Here is **the code in action**: on the left a fragment of the algorithm with **highlighted active lines**, on the right the data at that very moment. **The line color encodes the branch:** 🟡 active line, 🟢 match / write / found, 🔴 mismatch / jump, 🔵 index advance. Two phases — two panels; generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py).
...
## 9. Full step-by-step trace of the notes' example
...
## 11. Place in the string-algorithm series
