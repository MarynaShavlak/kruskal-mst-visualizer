...
Here is the implementation from the lecture notes — the one we dissect line by line. `counting_sort` sorts by **one** digit, and `radix_sort` calls it for each digit of the largest number (the fully documented version is in [`radix_sort/core.py`](radix_sort/core.py)):
...
> The code is kept verbatim from the notes (with the original Ukrainian comments). What each line does in `radix_sort`:
...
This is the version that produces the bucket distributions you see below — and they match the notes **exactly**. (Both versions, like the instrumented `radix_sort_steps`, give the same result as Python's built-in `sorted()` — this is checked by the [tests](tests/test_core.py).)
...
The bucket distribution and the array after gathering (printed by [`examples/01_intro.py`](examples/01_intro.py)):
...
The same line by line, with code highlighting and the `count[]`/`output` state (generated by [`examples/02_counting_sort.py`](examples/02_counting_sort.py)):
...
We build this for the array from the notes (its bucket distributions match the notes); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py). The grid shows the outer LSD loop — for each digit, "made buckets → gathered":
