 ...
 * плавного введення й комітить розпарсене ціле (стор санітизує до ≥0). Порозрядне
 * сортування працює з невід'ємними цілими.
 */
      ...
      {values.map((v, i) => (
        <div key={i} className="flex flex-col items-center gap-1">
          ...
          <div className="group relative">
            <NumberField
              value={v}
              aria={t("editor.rxAriaValue", { i })}
              onCommit={(n) => updateValue(i, n)}
            />
            <button
              type="button"
              aria-label={t("editor.rxDeleteAt", { i })}
...
function NumberField({
  ...
  return (
    <input
      ...
      className="w-16 rounded border bg-transparent px-1.5 py-1 text-center text-sm tabular-nums outline-none focus:ring-2 focus:ring-ring"
    />
  )
}
