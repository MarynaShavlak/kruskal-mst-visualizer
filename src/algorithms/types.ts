...
/**
 * Вид передумови коректності алгоритму — той інваріант вхідних даних, без якого
 * результат хибний (або, для графа, перетворюється на остовний ліс). Споживач
 * (живий детектор у шапці) за `kind` обирає відповідний стор-хук і lib-перевірку.
 */
export type PreconditionKind = "sorted-array" | "connected-graph"

/**
 * Декларативна передумова на дескрипторі алгоритму. Поки несе лише `kind`; підписи
 * (заголовок/ОК/попередження) — спільні chrome-ключі `precond.*`, не дублюються тут.
 */
...
export interface Algorithm {
  ...
  /**
   * Передумова коректності (опц.) — інваріант вхідних даних. Живить картку
   * передумов і живий детектор у шапці розділу. Алгоритми без передумови (напр.
   * лінійний пошук) поле не задають.
   */
