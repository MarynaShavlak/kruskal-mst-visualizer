...
Here is the implementation from the notes — the one we dissect line by line (the full version with docstrings is in [`selection_sort/core.py`](selection_sort/core.py)):
...
The learning version [`selection_sort_steps`](selection_sort/core.py) repeats this code **action for action**, but after every significant event it records a snapshot of the array and the comparison and swap counters — all the pictures below are built from those snapshots.
...
The full journal of all the passes (printed by [`examples/01_intro.py`](examples/01_intro.py)):
...
The summary from the console (printed by [`examples/01_intro.py`](examples/01_intro.py)):
...
**How to fix it — the stable variant via shifting.** Instead of the swap `arr[i] ↔ arr[min_idx]`, the minimum is **lifted out**, the elements of the block `[i..min_idx-1]` are **shifted right** by one position, and the minimum is inserted at position `i` (implementation — [`selection_sort_stable`](selection_sort/core.py)):
...
We build this for the array from the notes `[5, 3, 8, 4, 2]` (its line-by-line trace matches the one in the notes); generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py):
