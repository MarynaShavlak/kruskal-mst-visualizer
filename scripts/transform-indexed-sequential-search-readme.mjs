// Одноразовий трансформер: README Python-проєкту algo-indexed-sequential-search →
...
// (підсекції, кроки трасування) лишаються як є.
//
// Запуск:  node scripts/transform-indexed-sequential-search-readme.mjs [шлях-до-репо]
import { readFileSync, writeFileSync } from "node:fs"

const SRC = process.argv[2] ?? "/home/m-shavlak/WebstormProjects/algo-indexed-sequential-search"
const OUT = "src/algorithms/indexed-sequential-search/learn"
...
/**
 * Екранує `[` всередині alt-тексту зображень `![alt](url)`. Деякі підписи містять
 * діапазони на кшталт `[4, 8)`, а вкладена `[` ламає розбір зображення у
 * react-markdown (рендериться як текст + посилання). Екранування зберігає підпис і
 * лишає коректним `<img>`, який figureForSrc підмінює живим віджетом.
 */
